...
!   Actual subroutine that solves the equilibrium prices and calculates the 
!       distance between model and data.
!
! Data Dictionary:
!   Input:  np: number of market clearing conditions.
!           x: initial guess (as input).
!   Output: x: value of calibrated parameters (as output)
!           fvec: excess demand in each market
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    agginv = delta*pm*(muu*aggku+mur*aggkr+muf*aggkf)
    ...
    !if (myrank == root) then
    !    write (*,*) 'aggcm = ', aggcm
    !    write (*,*) 'manu   = ', manufacture
    !    write (*,*) 'export = ', export
    !    write (*,*) 'aggkm = ', aggkm
    !    write (*,*) 'aggkf = ', aggkf
    !    write (*,*) 'aggtax = ', aggtax
    !    write (*,*) 'vattax = ', vattax
    !    write (*,*) 'businesstax = ', businesstax
    !    write (*,*) 'personaltax = ', personaltax
    !endif
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
!   Calculate CPI
...
!!    sumsqfunc(6 ) = (aggtax/gdp - 0.08_dp)     ! Modified by Xin Tang
    ...
    sumsqfunc(1 ) = (calimodel(1) - cali_lowca_c)
    sumsqfunc(2 ) = (calimodel(2) - cali_cs_c)
    sumsqfunc(3 ) = (calimodel(3) - cali_cm_c)
    sumsqfunc(4 ) = (calimodel(4) - cali_gini_r)
    sumsqfunc(5 ) = (calimodel(5) - cali_gini_u)
    sumsqfunc(6 ) = (calimodel(6) - cali_tax_gdp)
    !sumsqfunc(6 ) = (vattax/aggtax - 0.45_dp)
    sumsqfunc(7 ) = (calimodel(7) - cali_corp_tax)
    sumsqfunc(8 ) = (calimodel(8) - cali_inc_tax)
    sumsqfunc(9 ) = (calimodel(9) - cali_cs_y)
    sumsqfunc(10) = (calimodel(10) - cali_cm_y)
    sumsqfunc(11) = (calimodel(11) - cali_ex_y)
