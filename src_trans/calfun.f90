...
!===============================================================
    ...
    integer  :: indi
	real(dp) :: xtol,epsfcn,factor,pinc,d
...
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    ...
    if ( equilibrium ) then
        ...
        if ( extend ) then
           ...
           open(1,file='example_in_42_cit_22ext.txt',form='formatted')
           do indi = 1,ntran_in
           ...
           open(1,file='example_in_42_vat_22ext.txt',form='formatted')
       ...
       if ( extend ) then
           ...
           ! use solution with smaller t as initial guess
           open(1,file='example_in_42_pit_22ext.txt',form='formatted')
           do indi = 1,ntran_in
               read(1,918) (eprice_in(indi,j), j=1,5)
           enddo
           close(1)
        ...
        ! old ss
        eprice(1,1) = 20.5133438772_dp
        eprice(1,2) = 18.8391036004_dp
        eprice(1,3) =  6.0878980669_dp
        eprice(1,4) =  3.8278637784_dp
        eprice(1,5) =  0.0074103014_dp                   
       endif
    endif
!%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    do j = 1,5
...
!if (myrank .eq. root) print*, p(jj), jj
    ...
    if ( equilibrium ) then
       ...
       !            factor,nprint,info,nfev,fjac,ldfjac,rr,lr,qtf,wa1,wa2,wa3,wa4)
        ...
        if ( myrank .eq. root) then
            write (*,'(A18,ES9.1)') ' Tolerance Level = ', xtol
        endif
       ! call fcn(nval,p,fvec,iflag)
       call hybrd(fcn,nval,p,fvec,xtol,maxfev,ml,mu,epsfcn,diag,mode2,&
                   factor,nprint,info,nfev,fjac,ldfjac,rr,lr,qtf,wa1,wa2,wa3,wa4)       
